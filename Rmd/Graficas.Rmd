    ...
    out <- tryCatch(
        {
          ...
          ann_text<-data.frame(fecha=df$fecha,
                               median=rep(limite ,nrow(df)),
                               candidato=df$candidato,
                               label=format(df$pctpropobs,digits=2),
                               index=seq(1,nrow(df)))
          ann_text <- ann_text %>% mutate(label=ifelse(as.numeric(substr(as.character(fecha),15,16)) %% 15, "",as.character(label))) %>% mutate(label=ifelse(label=='NA','',label))
          ggplot(df, aes(fecha,median, color=factor(candidato))) +
            ...
            geom_text(data = ann_text,aes(x=fecha,y=median,label=label), size = 3, alpha = 0.1, inherit.aes = FALSE)
        },
        error=function(cond) {
