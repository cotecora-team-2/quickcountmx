...
sample_diputados <- readr::read_delim("REMESAS0400020027.txt", 
...
estimates <- bootstrap_diputados(sample_diputados, stratum = distrito_f,
                                 ...
                                 coalitions_tbl = coalitions_tbl, B = 100, seed = 12,
                                 samples_table = TRUE)

national_vote_estimation <- estimates$total_tbl |> group_by(party) |> 
  summarise(inf = quantile(prop, 0.02), sup = quantile(prop, 0.98))
national_vote_estimation |> mutate(across(where(is.numeric), ~round(.x, 4))) |> 
  arrange(desc(sup))
```
...
majority_seats_rep <- assign_majority(estimates$strata_tbl, assignment_tbl , 
  ...
  summarise(n_seats_maj = sum(is_majority), .groups = "drop_last")

majority_seats_rep |>
  summarise(inf = quantile(n_seats_maj, 0.02), sup = quantile(n_seats_maj, 0.98)) |>
  arrange(-inf)
```
Total seats, without checking over-represntation

```{r}
estimates$total_tbl |> 
  filter(stringr::str_detect(party, "CNR|NULOS|part|CI", negate = TRUE)) |> 
  group_by(rep) |> 
  filter(prop >= 0.03) |> 
  mutate(
    prop_adj = prop / sum(prop),
    n_seats_prop = 200 * prop_adj) |> 
  ungroup() |> 
  right_join(majority_seats_rep, by = c("rep" = "rep", "party" = "candidate")) |> 
  mutate(
    n_seats_prop = ifelse(is.na(n_seats_prop), 0, n_seats_prop),
    n_seats_total = n_seats_prop + n_seats_maj) |> 
  group_by(party) |> 
