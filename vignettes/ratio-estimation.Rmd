...
assignment_tbl <- readr::read_csv("../data-raw/asignacion-coaliciones.csv") |> 
  rename(party = nombre_partido) |> mutate(strata = paste(ID_ENTIDAD, DISTRITO, sep = "."))


  
sample_diputados <- readr::read_delim("../data-raw/REMESAS0400020027.txt", 
...
```{r}
party_names <- estimates$strata_tbl$party |> unique()
assign_majority(estimates$strata_tbl, assignment_tbl , 
                party_name = party, candidate_name = candidato) |> 
  group_by(rep, party) |> 
  count() |> 
  group_by(party) |> 
  summarise(inf = quantile(n, 0.02), sup = quantile(n, 0.98)) |> 
  arrange(desc(sup)) |> 
  right_join(tibble(party = party_names)) |> 
  mutate(inf = ifelse(is.na(inf), 0, inf), sup = ifelse(is.na(sup), 0, sup)) 
```
