...
estimates <- hb_estimation(sample, stratum = distrito_f,
...
```{r, messages = FALSE, warning = FALSE}
# consider observed arrival times
data("arrivals_tbl")
sample_censored <- conteo %>% 
  left_join(arrivals_tbl %>% select(CLAVE_CASILLA, time)) %>%
  filter(!is.na(time)) %>% 
  filter(time < 2) 
# 3 hours later gives around 50% of sample:
prop_obs <- 0.50
estimates <- hb_estimation(sample_censored, stratum = distrito_f,
                      id_station = no_casilla,
                      sampling_frame =  conteo,
                      parties = all_of(c("AMLO", "JAMK", "RAC", "CAND_IND_02", "OTROS")),
                      covariates = .fittedPC1,
