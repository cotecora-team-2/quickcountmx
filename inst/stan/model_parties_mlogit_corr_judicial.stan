...
// party vote
  ...
  for(i in 1:N_f){
    ...
    if(in_sample[i] == 1){
      ...
      theta_f[1] = inv_logit((pred_f[1] + w_bias[1]));
      //alpha_bn_f =  n_f[i] * theta_f_total_prop[i] * theta_f;
      alpha_bn_f[1] =  n_f[i] * theta_f[1];
      for(k in 1:p){
  ...
  sum_votes = sum(y_out);
  for(k in 1:p){
    prop_votos[k] = y_out[k] / total_nominal;
  }
